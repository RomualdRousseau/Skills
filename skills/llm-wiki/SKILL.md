---
name: llm-wiki
description: Knowledge base and documentation management using Markdown wikis optimized for LLM indexing, retrieval, and maintenance.
tags:
  - knowledge-base
  - wiki
  - documentation
...
created: YYYY-MM-DD
updated: YYYY-MM-DD
sources: [list of raw source filenames using format "[[raw-source-filename]]" that informed this page]
tags: [relevant tags]
---
```
...
When the user says "setup zensical" or "init documentation":

1. Copy `templates/zensical.toml` to the project root.
2. Copy `templates/justfile` to the project root (merging if one exists).
3. Ensure `.gitignore` contains `.build/`.
4. Inform the user they can now run `just serve`.

### Publish

When the user says "publish the wiki" or "prepare the build":

1. Run the `preprocess_wiki.py` script located in this skill's directory.
2. The script converts `[[page]]` wikilinks to standard Markdown links `[page](page.md)` in a `.build/docs/` directory using root-relative paths.
3. Ensure Zensical is configured to use `.build/docs/` as its `docs_dir`.
4. Append a log entry:
   ```
   ## [YYYY-MM-DD] publish
   Build directory prepared: .build/docs/
   ```
...
1. Read the agent instructions file (e.g., AGENTS.md or project guidance)
...
- **Trigger**: "Initialize the LLM wiki workspace"
- **Trigger**: "Ingest source documents into the wiki"
- **Trigger**: "Run the wiki linter to check for broken links"
- **Trigger**: "Preprocess the wiki for compilation"
