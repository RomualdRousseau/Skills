...
description: Perform interactive genomic variant analysis using the Variant Analysis Multi-Agent System. Use when starting a VCF analysis, checking status, or querying clinical assessments and population frequencies.
tags:
  - genomics
  - bioinformatics
  - vep
...
- **Firebase Token**: Execute the following command to get the token:
  ```bash
  uv run python .agents/skills/science/science-variant-analysis/scripts/get_firebase_token.py
  ```
...
- **Trigger**: "Analyze the VCF file at [path]"
- **Trigger**: "Get the status of my variant analysis"
- **Trigger**: "Query the clinical assessments of variants in [gene]"
- **Trigger**: "Fetch the final variant analysis report"
